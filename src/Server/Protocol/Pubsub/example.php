...
echo $sep;
echo var_export($pubsub->getPayloadModels(), true) . PHP_EOL;
...
if ($pubsub->validateMessage($payload)) {
    ...
    $result = $pubsub->resolve('publish', 'resolvePublish', ['payload' => ['topic' => 'topic.channel', 'message' => 'hello world']], $pubsub);
    ...
    var_dump($response->isValid(), $response->toArray());
} else {
